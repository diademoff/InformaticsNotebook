...
{
    /// <summary>
    /// Collection of computer components, images and their definitions
    /// </summary>
    public static class DataCollection
    {
        ...
        public static DataElement[] GetAllElementsWhere(bool ImageExists, bool DefenitionExists)
        {
            List<DataElement> res = new List<DataElement>();
            foreach (var item in DataElements)
            {
                if (ImageExists && item.Images.Length == 0)
                {
                    continue;
                }
                if (DefenitionExists && item.Defenitions.Length == 0)
                {
                    continue;
                }
